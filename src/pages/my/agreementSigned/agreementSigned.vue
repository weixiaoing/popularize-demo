<script lang="ts" setup>
import { getSigned, getUserInfo } from '@/services/userAPI'
import type { userType } from '@/types'
import { onShow } from '@dcloudio/uni-app'
import { ref } from 'vue'

let info = ref<userType>()
const goSigned = async () => {
  const res = await getSigned()
  if (res.code == 200) {
    uni.showToast({
      title: '签约成功',
      icon: 'none',
      duration: 2000,
    })
    uni.navigateBack({ delta: 1 })
  }
}
onShow(async () => {
  const res = await getUserInfo()
  info.value = res.data
})
</script>
<template>
  <view class="container"
    ><h1>用户协议</h1>
    <p>
      <i>开发者</i
      >（以下简称“我们”）依据本协议为用户（以下简称“你”）提供<i>小推达人平台</i>服务。本协议对你和我们均具有法律约束力。
    </p>
    <h4>一、本服务的功能</h4>
    <p>你可以使用本服务推广。</p>
    <h4>二、责任范围及限制</h4>
    <p>你使用本服务得到的结果仅供参考，实际情况以官方为准。</p>
    <h4>三、隐私保护</h4>
    <p>
      我们重视对你隐私的保护，你的个人隐私信息将根据《隐私政策》受到保护与规范，详情请参阅《隐私政策》。
    </p>
    <h4>四、其他条款</h4>
    <p>4.1 本协议所有条款的标题仅为阅读方便，本身并无实际涵义，不能作为本协议涵义解释的依据。</p>
    <p>4.2 本协议条款无论因何种原因部分无效或不可执行，其余条款仍有效，对双方具有约束力。</p>
    <br />
    <button v-if="info?.contractStatus == 0" @click="goSigned">确认签约</button>
  </view>
</template>

<style lang="sass" scoped></style>
